...
exports.handler = async function(event) {
    ...
    try {
        ...
        if (!event.body) {
            ...
            throw new Error('El cuerpo de la solicitud (event.body) llegó completamente vacío.');
        }
        ...
        try {
            ...
            // Mensaje de error más específico.
            throw new Error(`Fallo al analizar el JSON. El texto recibido no es un JSON válido. Contenido: ${event.body}`);
        }

        const { text, page } = parsedBody;
        ...
        if (!text || typeof text !== 'string' || text.trim() === '') {
            console.log(`Solicitud para la página ${page} ignorada por falta de texto válido.`);
            return {
                statusCode: 200,
                body: JSON.stringify([]),
            };
